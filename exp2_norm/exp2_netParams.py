...
Network parameters for Experiment 2: Spine elimination effect on D3 test.
...
# Define numCells_long here at the top level
numCells_long = 20
...
# Configure stimuli for Experiment 2
rate = {'4K': 50, '12K': 50} # Hz for D3 test stimuli

# Long-range input populations 
longPops = ['S1_4K','S1_12K', 'OC']
# Use interval-based VecStim configuration which is more reliable
rate = {'4K': 50, '12K': 50} # Hz for D3 test stimuli

# Set start and end times for stimulation
start_4K = cfg.d3test_4k_stim_start
end_4K = cfg.d3test_4k_stim_end

start_12K = cfg.d3test_12k_stim_start
end_12K = cfg.d3test_12k_stim_end

pulses_4K = [{'start': start_4K, 'end': end_4K, 'rate': rate['4K'], 'noise': 0}]
pulses_12K = [{'start': start_12K, 'end': end_12K, 'rate': rate['12K'], 'noise': 0}]

# Configure VecStim populations
netParams.popParams['S1_4K'] = {'cellModel': 'VecStim', 'numCells': numCells_long,
                                    'ynormRange': layer['longS1_4K'], 'spkTimes': [cfg.duration + 200], 'pulses': pulses_4K}           

netParams.popParams['S1_12K'] = {'cellModel': 'VecStim', 'numCells': numCells_long,
                                    'ynormRange': layer['longS1_12K'], 'spkTimes': [cfg.duration + 200], 'pulses': pulses_12K}        
...
#------------------------------------------------------------------------------
# Connectivity - same pattern as original but simplified for Exp2
#------------------------------------------------------------------------------
...
for ipost, postType in enumerate(postTypes):
    weights = [5]*num_PT*num_dict[postType]
        ...
        for ipost, postCellType in enumerate(postCellTypes):
            ...
            if postCellType == 'PT':
                if sec in ['soma', 'perisom']:
                    sec = 'soma'
                    weights = [5]*num_PT*num_dict[preCellType]
                    locs = [
                ...
                if preCellType == postCellType:
                    weights = [5]*(num_dict[preCellType]-1)*num_dict[postCellType]
                    locs = [
                    ...
                    weights = [5]*num_dict[preCellType]*num_dict[postCellType]
                    locs = [
                        randomlist['II'][postCellType][preCellType][j][i][0][0]
                        for i in range(num_dict[preCellType])
                        for j in range(num_dict[postCellType])
...
#------------------------------------------------------------------------------
longPops = ['S1_4K','S1_12K']
cellTypes = ['PT']
for longPop in longPops:
    for secName in seclist['PT']['dend']:
        weights = [[5]*math.ceil(nseg['PT'][secName]/numCells_long)]*numCells_long*num_PT
        weights_syns = [[x, x] for x in weights]
        locs = [
            [syn[0] for syn in randomlist['LongE'][longPop][secName][j][i]]
...
print(f"Experiment 2 Network Parameters Loaded (preserving original structure).") 