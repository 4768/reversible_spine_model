...
if rank == 0:
    print(f'Using logs from {DATA_DIR}')
# ------------------------------------------------------------------------------
# Experiment Setup
# ------------------------------------------------------------------------------
...
def load_spine_weights(period_label=None):
    ...
    for node in range(40):
        ...
        #os.path.join(DATA_DIR, "weight", f"weight_node_{node}_{cfg.rdseed}.txt") #to be changed to weight_summary_node_{node}.txt
...
for spine_id in weights_before_d1:
    if spine_id in weights_after_d1:
        ...
        if weight_before >= cfg.spineEliminationThreshold and weight_after < cfg.spineEliminationThreshold:
            # This is control - keep the weight from D1->D3
