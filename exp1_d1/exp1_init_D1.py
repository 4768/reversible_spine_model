...
Implementation of Experiment 1 D1test phase: Coherent vs. Incoherent Spine Pair Stimulation
...
specifically for the D1test phase of Experiment 1.
"""
...
phase = 'D1test'  # Set the phase for this experiment
...
# D1test specific constants
D1TEST_WEIGHT_PERIOD = 'After D1 train'
D1TEST_ACTIVITY_TIMEPOINT = 'D1 test 4k'
...
def load_spine_weights():
    ...
    spine_weights_data : dict
        ...
        with nested dicts for periods. e.g., {'100.Adend1.0.5': {'After D1 train': 3.5}}
    """
...
def main_setup(selected_phase=None):
    ...
    For D1test_experiment, this function only processes D1test phase data.
    
    Parameters:
    ----------
    selected_phase : str or None
        If specified ('D1test'), only processes data for that phase.
        If None, defaults to 'D1test' for this specialized script.
    ...
    # Override selected_phase to ensure we only process D1test
    selected_phase = 'D1test'

    start_time = time.time()
    if rank == 0:
        print("="*40)
        print("Starting Experiment 1 D1test Setup")
        print("="*40)
...
#######################################################################
    ...
    if not candidates:
        if rank == 0: print(f"Error: No valid candidate spines found for D1test phase. Aborting setup.")
        comm.Abort(1); return None

    print(f"\nRank{rank}: Total candidate reference spines identified for D1test: {len(candidates)}")
    ...
    if rank == 0:
        ...
        print("\n" + "="*40)
        print(f"D1test Experiment Setup Completed in {elapsed_time:.2f} seconds")
...
if __name__ == "__main__":
    if rank == 0:
        print("*"*60)
        print(" This script (exp1_init_D1.py) is intended to be imported.")
        print(" Call 'main_setup()' from a main simulation script ")
        print(" (e.g., run_experiment_D1.py) AFTER basic sim/net initialization ")
