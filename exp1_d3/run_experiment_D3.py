...
"""
run_experiment_D3.py

Runner script for Experiment 1 D3test phase to evaluate whether coherent spine pairs 
enhance dendritic responses to conditioned stimuli (CS) compared to incoherent pairs.

This script runs specifically the D3test phase only:
1. Analyzes existing weight and activity data to identify coherent and incoherent spine pairs for D3test
2. Creates direct stimulation sources connecting to reference spine and group members
3. Loads weights from the appropriate timepoint (Before D3 test)
4. Runs simulation for D3test phase only
5. Compares dendritic responses and saves results to output files

Usage:
    python run_experiment_D3.py
            ...
            for conn in cell.conns:
                ...
                if spine_id in weights_dict:
                    if hasattr(conn['hObj'], 'weight'):
                        conn['hObj'].weight[0] = weights_dict[spine_id]['Before D3 test']
                        modified_count += 1
                else:
...
def main():
    ...
    if rank == 0:
        print(f"\n=== Running Experiment 1 D3test Phase: Coherent vs. Incoherent Spine Pair Testing ===")
        print(f"Output will be saved to: {cfg.saveFolder}")
        print(f"Loading weights from: Before D3 test")
    ...
    # Now call the main setup function from exp1_init_D3
    # This will set up only the D3test phase
    if rank == 0:
        print("Running Experiment 1 setup for D3test phase...")

    exp1_init_D3.main_setup('D3test')

    if rank == 0:
        print("Experiment 1 D3test setup complete. Proceeding with simulation...")
    ...
    sim.runSimWithIntervalFunc(20, exp1_init_D3.record_simulation_data)
    

    if rank == 0:
        print(f"=== Experiment 1 D3test finished successfully ===")
        print(f"Results saved to: {cfg.saveFolder}")
        print(f"The simulation tested D3test phase with coherent and incoherent stimulation.")
