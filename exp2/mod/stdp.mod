...
2025jan02: Added HSP (Homeostatic Synaptic Plasticity) mechanism to adjust weight based on postsynaptic voltage
ENDCOMMENT

NEURON {
    POINT_PROCESS STDP : Definition of mechanism
    RANGE HSPchange : whether postsyn g changed by synweight HSP
    ...
    :POINTER v_postsyn : Pointer to the postsynaptic voltage (HSP)
    RANGE V, Vtrg, Eenable, Etau, NEproc
    :POINTER proc_num
}
...
PARAMETER {
	...
	Etau=1e3 (ms)			: // HSP time constant
	Eenable=0			: // enable HSP
    HSP_type = 1
    NEproc
    HSPchange = 0
}
...
BREAKPOINT {
    :SOLVE HSP_update METHOD after_cvode
}
...
NET_RECEIVE (w) {
    ...
    :else {
        :HSP, weight modification randomly
...
PROCEDURE adjustweight(wc) {
   ...
   if (verbose > 0 ) { printf("adjusted: t=%f, synweight=%f",t,synweight) }
}

:PROCEDURE HSP_update() {
:	LOCAL sf, w, raww
:	: here we cal weight based on postsyn V but whether apply to change depends on random process in Net_receive
	: ---------
...
:	sf = Vtrg - V
:	if (Eenable) {
:        raww = exprand(sf)/Etau
:        if (softthresh == 1) {w = softthreshold(raww)}
:		synweight = synweight * (1 + w)
:	}
:    if (synweight < 0) { synweight = 0 }
:    if (synweight > wmax) { synweight = wmax }
:}
